    ...
    <div className="container my-4">
      ...
      <div className="d-flex justify-content-around mb-4">
        <div id="userCard" className="card">
          <img id="userCarImg" className="card-img-top" src={userCar?.image} alt="User Car" />
        ...
        <div id="computerCard" className="card">
          <img id="computerCarImg" className="card-img-top" src={computerCar ? computerCar.image : `${process.env.PUBLIC_URL}/media/dummy-photo.jpg`} alt="Computer Car" />
          <div className="card-body">
            <h5 id="computerCarTitle" className="card-title">{computerCar ? computerCar.name : '???'}</h5>
            <ul id="computerCarText" className="card-text">
              <li>Year: {computerCar ? computerCar.year : '???'}</li>
              <li>Top Speed: {computerCar ? computerCar.topSpeed : '???'} km/h</li>
              <li>Horsepower: {computerCar ? computerCar.horsepower : '???'} hp</li>
              <li>Acceleration: {computerCar ? `0-100 km/h in ${computerCar.acceleration} seconds` : '??? seconds'}</li>
              <li>Engine Displacement: {computerCar ? computerCar.cc : '???'} cc</li>
            </ul>
          </div>
        </div>
      </div>
      <div id="attributeButtons" className="text-center">
        <button className="btn btn-danger" onClick={() => playTurn('topSpeed')}>Top Speed</button>
        <button className="btn btn-danger" onClick={() => playTurn('horsepower')}>Horsepower</button>
        <button className="btn btn-danger" onClick={() => playTurn('acceleration')}>Acceleration</button>
        <button className="btn btn-danger" onClick={() => playTurn('cc')}>Engine Displacement</button>
      </div>
      {gameResult && (
